...
func _get_node_from_view(node_view: TreeItem) -> Node:
  ...
  while parent_view.get_parent() != null:
    ...
    # Exclude root
    if parent_view.get_parent() == null: break
...
func pick(mpos: Vector2) -> void:
  ...
  if node != null:
    log.pp("Picked ", node, " at ", node.get_path())
    _focus_in_tree(node)
  else:
...
#        s = str(s, "  ")
#
#    log.pp(s, "Looking at ", root, ": ", root.name)
  var node: Node = null
  ...
  for i in root.get_child_count(true):
    ...
    if (child_canvas_item != null and not child_canvas_item.visible):
      ...
      continue
    if child is Viewport: continue
    if child == _control_highlighter: continue
...
func _on_ShowInInspectorCheckbox_toggled(_unused_button_pressed: bool) -> void: pass
...
func _on_popup_menu_id_pressed(id: int) -> void:
  ...
  match id:
    ...
    POPUP_ACTIONS.COPY_PATH_TO_CLIPBOARD:
      ...
      log.pp("Copied to clipboard: %s" % [node.get_path()])
    ...
    POPUP_ACTIONS.COPY_NODE_TYPES_TO_CLIPBOARD:
      ...
      log.pp("Copied to clipboard: %s" % [node_types_str])
    ...
    POPUP_ACTIONS.COPY_NODE_CHILD_PATH_INDICES:
      ...
      log.pp("Copied to clipboard: %s" % [string_path])
