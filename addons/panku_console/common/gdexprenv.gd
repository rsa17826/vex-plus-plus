...
static func get_export_properties_from_script(script: Script):
  ...
  for d in data:
    if !(d.usage == PROPERTY_USAGE_SCRIPT_VARIABLE | PROPERTY_USAGE_EDITOR | PROPERTY_USAGE_STORAGE): continue
    result.append(d)
  return result
...
static func get_object_outline(obj: Object) -> String:
  ...
  for p in properties:
    if p.usage & PROPERTY_USAGE_SCRIPT_VARIABLE == 0: continue
