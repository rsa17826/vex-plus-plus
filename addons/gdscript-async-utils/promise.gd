...
func reject(reason: String):
	resolve(Promise.Error.create(reason))
...
static func async_all(funcs: Array) -> Array:
	if funcs.is_empty():
		return []
	return await AllAwaiter.new(funcs)._promise.wait()
...
static func async_any(funcs: Array) -> Variant:
	return await AnyAwaiter.new(funcs)._promise.wait()
...
static func async_race(funcs: Array) -> Variant:
	return await RaceAwaiter.new(funcs)._promise.wait()
